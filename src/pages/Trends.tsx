  ...
  return (
    <div className="stack">
      <header className="page-head">
        <p className="eyebrow">D · Prediction</p>
        <h1>Where registrations are heading</h1>
        <p className="lede">
          Linear regression on 2023–2025 MOT totals. A prototype baseline — not a
          causal forecast.
        </p>
      </header>
          ...
          {YEARLY_TOTALS.map((y) => (
            <div key={y.year} className="year-col">
              <div
                ...
                style={{ height: `${(y.total / maxTotal) * 180}px` }}
              />
          ...
          {forecast.map((y) => (
            <div key={y.year} className="year-col">
              <div
                ...
                style={{ height: `${(y.predicted / maxTotal) * 180}px` }}
              />
      ...
      <section className="panel">
        ...
        <table>
          ...
          <tbody>
            {byClass.map((row) => (
              <tr key={row.classId}>
                <td>{CLASSES[row.classId].label}</td>
      ...
      <StepFooter step="trends" nextLabel="Continue to energy estimation" />
    </div>
  )
}
